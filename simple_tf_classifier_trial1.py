...
class CSV_Image_Semantic_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        vector = torch.tensor([self.glove_vectors[row["class_sub"]],
                               self.glove_vectors[row["class_obj"]]], dtype=torch.float32)
...
class Image_Semantic_AwareClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat(self.latent_extractor(x[0]), x[1])
        x = self.classifier(x)
        return x
...
def test_loop(dataloader, model, loss_fn):
    size = len(dataloader.dataset)
    test_loss, correct, true_count = 0, 0, 0
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    model = ImageAwareClassifier()  # todo 모델이 가정이 너무 이상할 수도 있으니까 vanilla version도 짠다.
    model.to(device)
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.1, 0.9]).to(device))
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)

    full_dataset = CSV_Image_Dataset(root_dir=dataset_path, data_sub_dir=['input_image_raw_trim-use','input_image_depth_trial2-use','prometeus-use'])
