...
class CSV_Image_Dataset(Dataset):
    ...
    def _create_mask(self, row, H, W):
        ...
        sub_mask[row["ymin_sub"]:row["ymax_sub"]+1, row["xmin_sub"]:row["xmax_sub"]+1] = 1
        obj_mask[row["ymin_obj"]:row["ymax_obj"]+1, row["xmin_obj"]:row["xmax_obj"]+1] = 1
    ...
    def __getitem__(self, idx):
        ...
        depth = cv2.imread(row["depth_path"], cv2.IMREAD_ANYDEPTH)
        ...
        dps = tensorize_transform(
            np.concatenate((np.expand_dims(depth, axis=-1),
                            np.expand_dims(sub_mask, axis=-1), np.expand_dims(obj_mask, axis=-1)),
                           axis=-1)
        )

        x = torch.cat((rgb, dps), dim=-1)  # (3, H, W) + (3, H, W) = (6, H, W)
        x = torch.permute(x, (2, 0, 1)).unsqueeze(0)  # (H, W, 6) -> (1, 6, H, W)
...
class ImageAwareClassifier(nn.Module):
    def __init__(self):
        ...
        _first_feature_layer = [
            nn.Conv2d(5, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),
        ...
        self.classifier = nn.Sequential(*_classifier, *_last_feature_layer)
...
if __name__ == "__main__":
    random_seed = 777
    dataset_path = r'Z:\assistant\dataset\prometeus'
    train_size = 0.8

    torch.manual_seed(random_seed)
    model = SimpleClassifier_v1()  # todo 모델이 가정이 너무 이상할 수도 있으니까 vanilla version도 짠다.
    model.to(device)
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.1, 0.9]).to(device))
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-3)

    full_dataset = CSV_Dataset(root_dir=dataset_path)
