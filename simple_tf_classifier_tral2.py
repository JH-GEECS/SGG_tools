...
class CSV_Image_Dataset(Dataset):
    def __init__(self, root_dir, data_sub_dir):
        ...
        for each_csv in self.csv_files:
            ...
            each_rgb_image_path = os.path.join(root_dir, data_sub_dir[0], image_path_raw[0] + "_" + image_path_raw[1] + ".jpg")
            each_depth_image_path = os.path.join(root_dir, data_sub_dir[1], image_path_raw[0] + "_" + image_path_raw[1] + ".png")
...
class CSV_Image_Semantic_Dataset_Deployer(Dataset):
    def __init__(self, root_dir, data_sub_dir, model):
        """
    ...
    def __getitem__(self, idx):
        ...
        for index, row in df.iterrows():
            ...
            pred = (torch.argmax(self.model([x, vector]), dim=1)).cpu().numpy().astype(bool)
            row["semantic"] = pred
...
if __name__ == "__main__":
    ...
    if train_model:
        ...
        model = Image_Semantic_AwareClassifier_v2()  # todo 모델이 가정이 너무 이상할 수도 있으니까 vanilla version도 짠다.
        model.to(device)
        loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.2, 0.8]).to(device))
        optimizer = torch.optim.Adam(model.parameters(), lr=lr)

        full_dataset = CSV_Image_Semantic_Dataset(root_dir=dataset_path, data_sub_dir=['input_image_raw_trim-use','input_image_depth_trial2-use','prometeus-use'])
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=16, num_workers=1, pin_memory=False, shuffle=True)
        test_dataloader = DataLoader(test_dataset, batch_size=16, num_workers=1, pin_memory=False, shuffle=False)
        ...
        for t in range(epochs):
            ...
            torch.save(checkpoint, os.path.join(os.path.join(dict_path, str(t) + '_' + str(rtpt) + '_' + str(acc)), f'checkpoint_{rtpt}.pt'))

        print("Done!")

    else:

        model_checkpoint_path = r'C:\model_temp\checkpoint\checkpoint_0.7964601516723633.pt'

        model = Image_Semantic_AwareClassifier_v2()
        ...
        deploy = CSV_Image_Semantic_Dataset_Deployer(root_dir=dataset_path,
                                                     data_sub_dir=['input_image_raw_trim-use','input_image_depth_trial2-use','prometeus-deploy-trial1'],
                                                     model=model)
        deploy_loader = DataLoader(deploy, batch_size=1, num_workers=1, pin_memory=False, shuffle=False)

        for batch in deploy_loader:
            start = time.time()
            print('Processed CSV file:', batch[0], f'time counsumed : {time.time() - start}')
